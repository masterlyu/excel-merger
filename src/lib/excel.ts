...
/**
 * DocumentAnalyzer 결과를 SheetData 형식으로 변환하는 어댑터 함수
 * analyzeDocument의 결과를 UI 컴포넌트에서 사용하는 형식으로 변환합니다.
 */
function adaptAnalyzerResultToSheetData(
  jsonData: any[][], 
  sheetName: string, 
  analyzerResult: { 
    patternName: string,
  ...
  console.log(`[adaptAnalyzerResultToSheetData] 문서 분석 결과 변환: 패턴=${analyzerResult.patternName}, 헤더행=${analyzerResult.headerRowIndex}, 데이터시작=${analyzerResult.dataStartRowIndex}, 데이터종료=${analyzerResult.dataEndRowIndex}`);
  
  // 헤더 추출
  const headerRow = jsonData[analyzerResult.headerRowIndex] || [];
  ...
  // 데이터 행 추출 (시작 행부터 종료 행까지)
  const dataRows = jsonData.slice(
    analyzerResult.dataStartRowIndex, 
    analyzerResult.dataEndRowIndex + 1
  );
  ...
  // SheetData 객체 반환 (기존 형식과 호환됨)
  return {
    name: sheetName,
    headers: uniqueHeaders,
    data: data,
    totalRows: data.length,
    headerRow: analyzerResult.headerRowIndex + 1, // 1-기반 인덱스로 변환
    dataStartRow: analyzerResult.dataStartRowIndex + 1, // 1-기반 인덱스로 변환
    title: analyzerResult.patternName !== 'Standard Excel' ? 
      `문서 유형: ${analyzerResult.patternName}` : undefined
  };
}
  ...
  try {
    ...
    for (const sheetName of workbook.SheetNames) {
      ...
      try {
        ...
        // documentAnalyzer를 사용하여 문서 구조 분석
        const analyzerResult = analyzeDocument(jsonData);
        console.log(`[readExcelFile] 문서 분석 결과: `, analyzerResult);
        
        // 분석 결과를 SheetData 형식으로 변환
        const sheetData = adaptAnalyzerResultToSheetData(jsonData, sheetName, analyzerResult);
...
export async function getSheetData(fileId: string, sheetName: string): Promise<SheetData | null> {
  ...
  try {
    ...
    try {
      ...
      // 데이터 처리 (원본 유지)
      // 중요: 여기서는 특수 문서 감지 로직을 제거하고, 모든 데이터를 그대로 반환합니다
